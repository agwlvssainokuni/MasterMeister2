...
**（2026-07-15変更要求）** `GET /connections`（アクセス可能接続一覧）は削除した。
...
Application Design Q3=B）。フロントエンドの`SchemaTableListPage`はU1のグローバル接続
コンテキスト（`useConnection()`）を参照する形に変更した。
...
| テストクラス | 検証内容 |
|---|---|
| `MasterDataControllerTest` | 4エンドポイント（`listAccessibleSchemas`/`listAccessibleTables`/`listRecords`/`applyChanges`）それぞれについて認証済みユーザ成功系・未認証401を検証（本ユニットは管理者ロール制約を持たないため403系テストは対象外、example-based、8件。2026-07-15変更要求で`listAccessibleConnections`関連2件を削除） |
