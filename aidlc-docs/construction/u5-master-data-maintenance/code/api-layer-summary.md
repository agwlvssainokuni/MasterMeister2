...
## `MasterDataController`（`/api/master-data/{connectionId}`）

### `GET /schemas`（アクセス可能スキーマ一覧）
...
### `GET /schemas/{schema}/tables`（アクセス可能テーブル一覧）
...
### `POST /schemas/{schema}/tables/{table}/records:search`（レコード検索）
...
### `POST /schemas/{schema}/tables/{table}/records:apply`（レコード更新）
...
| テストクラス | 検証内容 |
|---|---|
| `MasterDataControllerTest` | 4エンドポイント（`listAccessibleSchemas`/`listAccessibleTables`/`listRecords`/`applyChanges`）それぞれについて認証済みユーザ成功系・未認証401を検証（本ユニットは管理者ロール制約を持たないため403系テストは対象外、example-based、8件） |
