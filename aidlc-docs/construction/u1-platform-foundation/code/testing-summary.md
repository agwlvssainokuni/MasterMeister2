...
- `store/connectionStore.test.ts`（6件）・`hooks/useConnection.test.ts`（3件）を新規作成、
  `components/AppLayout.test.tsx`に7件追加（既存3件→10件）、`routes/AppRouter.test.tsx`は
  `listAccessibleConnections`のモック化のみ対応（新規テストケース追加なし、既存4件のまま）。
  バックエンド変更なし（U1は本変更要求ではフロントエンドのみ）。
- `npx vitest run`（フロントエンド全体）で271/271件成功、`npx tsc -b`・`npx oxlint`も
  エラーなしを確認済み（本Step時点で実行・グリーン確認まで実施——U3同様、影響ユニット全体の
  最終確認はBuild and Testステージで改めて行う）。