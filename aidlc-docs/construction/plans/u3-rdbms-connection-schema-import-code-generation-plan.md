...
- [x] 11-1. `frontend/src/features/rdbmsConnection/` に
      ...
      エラーなしを確認。
- [x] 11-2. `frontend/src/features/schema/` に
      ...
      実装メモ: `types.ts`は`SchemaImportResult`/`TableMetadata`/`ColumnDetail`/`TableDetail`
      をバックエンドDTO（`SchemaImportResult.java`, `TableMetadata.java`, `ColumnDetail.java`,
      `TableDetail.java`）と1:1で定義。`schemaApi.ts`は`SchemaController`の4エンドポイント
      （`POST .../schema-import`, `GET .../schemas`, `GET .../schemas/{schema}/tables`,
      `GET .../schemas/{schema}/tables/{table}`）にそれぞれ対応し、スキーマ名・テーブル名は
      `encodeURIComponent`でパスエスケープ。`SchemaImportPanel`は`connectionId`と`onClose`を
      propsで受け取るモーダル/パネルとして実装し、取り込み実行→結果表示→閉じるの3ボタン構成
      （指定の2つのdata-testidに加え`-close-button`を追加）。`SchemaBrowserPage`は
      `useParams`で`connectionId`を取得し、マウント時に`listSchemas`を呼び出し、
      `SchemaSelector`選択変更時に`listTables`、`TableList`の行選択時に`getTableDetail`を
      呼び出す一方向データフローとした。`TableDetailPanel`は`primaryKeySequence`昇順で
      主キー列を並べ、`tableType === 'VIEW'`または主キー列が空の場合は「なし」と表示
      （`business-rules.md` 2.1準拠）。`ConnectionListPage.tsx`のプレースホルダーだった
      `handleImportSchema`を実結線し、`importPanelConnectionId`状態で`SchemaImportPanel`を
      条件付き表示するモーダル起動に変更（`frontend-components.md`の「独立ルートを持たず
      起動するパネル」設計どおり）。`AppRouter.tsx`/`AppLayout.tsx`への`SchemaBrowserPage`
      ルート追加は11-3/11-4のスコープのため未実施。`npx oxlint`・`npx tsc -b`いずれも
      エラーなしを確認。
- [x] 11-3. `frontend/src/routes/AppRouter.tsx`（既存、ブラウンフィールド修正）:
      ...
      起動するモーダル/パネルとして生成する。
      実装メモ: 既存の`/admin/audit-logs`・`/admin/pending-users`ルートと同じ
      `<ProtectedRoute requiredRole="ADMIN">`ラップパターンで4ルートを`Routes`内に追加。
      `SchemaImportPanel`は本項目でも独立ルートを追加せず（11-2で`ConnectionListPage.tsx`に
      実結線済み）、`frontend-components.md`ルーティング表の4パスと1:1対応。
      `npx oxlint`・`npx tsc -b`いずれもエラーなしを確認。
- [x] 11-4. `frontend/src/components/AppLayout.tsx`（既存、ブラウンフィールド修正）に
      ...
      実装メモ: 既存の「監査ログ」リンクと同一の
      `isAuthenticated && currentUser?.role === 'ADMIN'`条件分岐パターンを再利用し、
      `data-testid="app-layout-nav-rdbms-connections"`のリンクを`/admin/pending-users`と
      `/admin/audit-logs`の間に追加。`npx oxlint`・`npx tsc -b`いずれもエラーなしを確認。
      これでStep 11（フロントエンド生成）の全4項目が完了。
...
- [x] 12-1. Vitest + React Testing Library で以下のexample-basedテストを生成:
      ...
      実装メモ: 新規10ファイル（`api/connectionApi.test.ts`, `ConnectionTable.test.tsx`,
      `ConnectionFormPage.test.tsx`, `ConnectionListPage.test.tsx`,
      `api/schemaApi.test.ts`, `SchemaImportPanel.test.tsx`, `SchemaSelector.test.tsx`,
      `TableList.test.tsx`, `SchemaBrowserPage.test.tsx`, `TableDetailPanel.test.tsx`）
      ＋既存2ファイル修正（`AppRouter.test.tsx`に`/admin/rdbms-connections`・
      `/admin/schema/:connectionId`の未認証リダイレクトテストを追加、`AppLayout.test.tsx`に
      `app-layout-nav-rdbms-connections`の表示/非表示検証を既存3テストへ追加）。
      既存テスト（`PendingUsersPage.test.tsx`, `authApi.test.ts`, `PasswordSetupPage.test.tsx`等）
      の規約を踏襲: apiモジュールは`vi.mock`でモック（ページ/テーブル系）、api層自体は
      `globalThis.fetch`を`vi.spyOn`で直接検証、`useParams`/`useNavigate`を使う画面は
      `MemoryRouter`+`Routes`でラップ。`connectionApi`/`schemaApi`のGETリクエスト検証は
      `apiFetch`が`method`未指定時に`RequestInit`へ`method`キー自体を含めない実装のため、
      `expect.objectContaining({ method: undefined })`だと欠落キーとして不一致になることが
      `npx vitest run`実行時に判明——`fetchSpy.mock.calls[0][0]`でURLのみ照合する方式に修正。
      `TableDetailPanel`は`primaryKeySequence`昇順の複合主キー表示と、通常テーブルの主キー
      無し/ビューでの「なし」表示の3パターンを検証。`npx oxlint`・`npx tsc -b`・
      `npx vitest run`（114件、30ファイル）すべて成功を確認。

### Step 13: フロントエンドコンポーネントサマリ
- [x] 13-1. `aidlc-docs/construction/u3-rdbms-connection-schema-import/code/
      frontend-summary.md`を生成し、`features/rdbmsConnection/`・`features/schema/`の
      コンポーネント一覧・`data-testid`一覧を記載。
      実装メモ: `features/rdbmsConnection/`（types.ts, api/connectionApi.ts,
      ConnectionListPage.tsx, ConnectionTable.tsx, ConnectionFormPage.tsx）・
      `features/schema/`（types.ts, api/schemaApi.ts, SchemaImportPanel.tsx,
      SchemaBrowserPage.tsx, SchemaSelector.tsx, TableList.tsx, TableDetailPanel.tsx）の
      各ファイルの役割・API結線・data-testid一覧、追加ルーティング4パス、実装時判断事項
      （パスワード欄の空欄初期化、`/admin/schema/:connectionId`への導線が未設計のため
      未追加である旨、`testConnection`オーバーロード）、Step 12のテストカバレッジ表
      （新規10ファイル・41件＋既存2ファイル拡張、フロントエンド全体30ファイル・114件）を
      記載。U2の`frontend-summary.md`と同じ構成（既存資産拡張／新規feature／ルーティング
      一覧／data-testid一覧／実装時判断事項／テストカバレッジ）に準拠。

### Step 14: データベースマイグレーションスクリプト
- [x] 14-1. **該当なし（N/A）**: U1/U2と同様、内部DB(H2)のスキーマ管理はJPAの自動DDL生成に
      委ね、Flyway/Liquibase等は導入しない。
      確認メモ: 本Unitで追加した`RdbmsConnection`（Step 2-4）・`SchemaTable`/`SchemaColumn`
      （Step 2-9）の3エンティティもJPA自動DDL（`spring.jpa.hibernate.ddl-auto`）の対象であり、
      追加のマイグレーションスクリプトは不要（U1 NFR Design Question 5 = A を踏襲）。
      対象RDBMS（MySQL/MariaDB/PostgreSQL/H2、`NamedParameterJdbcTemplate`経由）側の
...
- [x] 15-1. Step 4/7/10/13で生成した各サマリに加え、
      ...
      再確認）を生成する。
      実装メモ: U2の`testing-summary.md`と同じ構成（PBT-10遵守状況／P1〜P12対応表／
      example-basedテスト一覧／実行確認状況）で生成。作成にあたり`./gradlew test`
      （29テストクラス・133/133件成功）と`npx vitest run`（30ファイル・114/114件成功）を
      実際に実行し、正確な件数を採取・記載した。その過程で`api-layer-summary.md`
      （Step 7）・`repository-layer-summary.md`（Step 10）記載の一部テスト件数
      （`RdbmsConnectionControllerTest` 15→実際18、`SchemaControllerTest` 10→実際12、
      `SchemaTableRepositoryTest` 8→実際9）に誤差があることを発見し、`testing-summary.md`内に
      正確な件数と差異の注記を記載した（実装方針自体に誤りはなく、件数記載のみの誤差）。
...
- [x] 16-1. `backend/src/main/resources/application.yml`（既存、ブラウンフィールド修正）に
      ...
      実装メモ: `mm.app.frontend.base-url`ブロックの直後に`rdbms-connection`ブロックを追加。
      `encryption-key`は`mm.app.jwt.secret`と異なりYAML上のデフォルト値を持たせず
      `${MM_APP_RDBMS_CONNECTION_ENCRYPTION_KEY}`のみとし（`tech-stack-decisions.md`の
      「既定未設定」要件に合わせ、`EncryptedStringConverter`の`@Value`側にもデフォルトが
      無いことを確認済み）、環境変数未設定時はプレースホルダ解決自体が失敗しfail-fastとなる。
      プール3キーは`ConnectionPoolRegistry`の`@Value`デフォルト（`5`/`0`/`5s`）と整合する値を
      環境変数プレースホルダ付きで設定（`connection-timeout`は`5000ms`表記、Spring `Duration`
      バインディングで`5s`と同値）。追記後`./gradlew build`（コンパイル・全テスト実行・check・
      assemble・bootWar）が成功することを確認した（テストは`spring.profiles.active=test`で
      `application-test.yml`の`encryption-key`を使用するため本変更の影響を受けない）。
