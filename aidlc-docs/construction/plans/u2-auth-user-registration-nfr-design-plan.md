...
A. 共通の`OpaqueTokenGenerator`コンポーネント（`cherry.mastermeister.userregistration`または`auth`
   ...
   Generationで具体化）の双方がこれを利用し、生成・ハッシュ化ロジックの重複を避ける。名称を
   `OpaqueTokenGenerator`とするのは、U1で既に導入済みの`JwtTokenProvider`（アクセストークン
   ＝JWT用）と明確に区別するため（単なる`TokenGenerator`ではアクセストークン/リフレッシュ
   トークン/登録トークンのいずれを指すか曖昧になる）（推奨）
