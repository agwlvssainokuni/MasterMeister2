...
- **A（推奨）**: 各タブに対応するレコードを持つ集約モデルとする。
  ```
  record QueryBuilderModel(List<SelectItem> selectItems, List<FromItem> fromItems,
  ...
  record FromItem(String schema, String table, String alias)
    // FROM タブは1件（ベーステーブル）、JOIN タブは複数件（joinItems）
...
- [ ] `aidlc-docs/construction/u6-query-builder/functional-design/domain-entities.md`
- [ ] `aidlc-docs/construction/u6-query-builder/functional-design/business-rules.md`
- [ ] `aidlc-docs/construction/u6-query-builder/functional-design/business-logic-model.md`
      （PBT-01: テスト可能な性質セクションを含む、`property-based-testing`拡張が有効なため）
- [ ] `aidlc-docs/construction/u6-query-builder/functional-design/frontend-components.md`