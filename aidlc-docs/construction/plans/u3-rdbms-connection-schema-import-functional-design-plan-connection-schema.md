...
## 実行チェックリスト
- [x] Step A: `business-rules.md`に「1.7 アクセス可能な接続一覧の取得」を追加し、
      フィルタ基準・認可要件（`hasRole("ADMIN")`ではなく認証済み全ユーザー）を明記した
- [x] Step B: `business-rules.md` 4章のAPI認可表に新規エンドポイントを追加した
- [x] Step C: `business-logic-model.md`に新規フロー6「アクセス可能な接続一覧の取得」と
      PBT性質P12を追加した
- [x] Step D: `frontend-components.md`に`listAccessibleConnections()`のapi.ts定義を追加し、
      呼び出し元がU1の`AppLayout`（本ユニットの画面ではない）である旨を明記した