...
- [ ] Step 1: ドメイン型へのschema追加 — `QueryHistory.java`（エンティティ、`schema`列
...
- [ ] Step 2: `QueryExecutionService.java`改修 — `EffectivePermissionResolver`を新規注入する。
...
- [ ] Step 3: `QueryExecutionController.java`改修 — `executeAdhocSql`/`executeSavedQuery`で
      `request.schema()`を渡す。新規エンドポイント`GET /{connectionId}/schemas`を追加する。
- [ ] Step 4: `QueryHistoryService.java`改修 — `recordExecution`で`QueryHistory`に`schema`を渡す。
      `toEntry`で`HistoryEntry`に`schema`を渡す。
- [ ] Step 5: バックエンドテスト改修・追加 — `QueryExecutionServiceTest.java`（既存テストへの
...
- [ ] Step 6: `queryExecution/api.ts`・`types.ts`改修 — `listAccessibleSchemas(connectionId)`を
      新規追加する。`executeAdhocSql`/`executeSavedQuery`に`schema`パラメータを追加する。
- [ ] Step 7: `QueryExecutionPage.tsx`改修 — `connectionId`の取得元を`savedQueryId`有無で分岐
...
- [ ] Step 8: `savedQuery/SavedQueryListPage.tsx`改修 — `connectionId`を`useConnection()`から
      取得する（URL受け取りを廃止）。「実行」リンクは`savedQueryId`のみを付与する
      （`connectionId`は付与しない）。
- [ ] Step 9: `savedQuery/SavedQuerySaveForm.tsx`改修 — `connectionId`を`useConnection()`から
      取得する（URL受け取りを廃止）。
- [ ] Step 10: `savedQuery/SavedQueryDetailPage.tsx`改修 — 「実行」ボタンの遷移先URLから
      `connectionId`パラメータを削除する（`savedQueryId`のみ）。
- [ ] Step 11: `queryHistory/types.ts`改修 — `HistoryEntry`に`schema`フィールドを追加する。
- [ ] Step 12: `queryHistory/QueryHistoryListPage.tsx`改修 — `connectionId`を`useConnection()`
...
- [ ] Step 13: フロントエンドテスト改修 — `SavedQueryListPage.test.tsx`・
...
- [ ] Step 14: Documentation Generation — `aidlc-docs/construction/u7-saved-query-execution-
