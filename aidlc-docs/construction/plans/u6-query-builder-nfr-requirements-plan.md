...
- [x] `aidlc-docs/construction/u6-query-builder/nfr-requirements/nfr-requirements.md`
- [x] `aidlc-docs/construction/u6-query-builder/nfr-requirements/tech-stack-decisions.md`
...
[Answer]: B（リストごとに個別の上限値を設定する。SELECT句は多め、JOIN句はSELECT句ほど不要）

---

## Question 4-2: リストごとの上限値の具体的な数値（Q4=Bを受けた追加確認）

Q4でB（リストごとに個別の上限値を設定、SELECT句は多め・JOIN句は少なめ）と回答いただいた
ことを受け、各リストの具体的な上限値を提案する。

| リスト | 設定キー | 提案既定値 | 根拠 |
|---|---|---|---|
| `selectItems` | `mm.app.query-builder.max-select-items` | `100` | 幅の広いテーブルの全カラム＋集計関数の組み合わせを許容するため、6リスト中最も多め |
| `joinItems` | `mm.app.query-builder.max-join-items` | `10` | 実務上のJOIN段数は数件程度に収まることが多く、`selectItems`ほどの規模は不要 |
| `whereConditions` | `mm.app.query-builder.max-where-conditions` | `30` | AND結合のみ（`business-rules.md` 2.1）のため、条件数が多くなりすぎないことを想定した中程度の値 |
| `groupByColumns` | `mm.app.query-builder.max-group-by-columns` | `30` | `selectItems`中の非集計カラムに対応する値のため`whereConditions`と同程度 |
| `havingConditions` | `mm.app.query-builder.max-having-conditions` | `20` | `whereConditions`と同様の性質だが集計後の絞り込みでありさらに少数を想定 |
| `orderByItems` | `mm.app.query-builder.max-order-by-items` | `20` | 並び替えキーは通常少数（多くても数件）にとどまるため |

いずれも`SqlGenerationService.generate`・`SqlParsingService.parse`（変換後モデル）の両方で
検証し、超過時は`ValidationException`で拒否する（Q4本文の検証タイミングを踏襲）。

A. 上記の提案値どおりに確定する（推奨）

B. 一部またはすべての数値を変更する（具体的な値を指定してください）
