...
### Q2. Business Logic Modeling — レコード一覧のデータ構造（`RecordDto`を廃止し`RecordListResult`へ）

対象テーブルは接続ごとに任意のカラム構成を持つため、固定フィールドのJavaクラスでは表現できない。
チャットでの検討の結果、以下の方針に確定（詳細は後日audit.mdに記録する検討経緯を参照）。

- **A（採用、検討の結果の修正版）**: JDBCの`ResultSet`が「行データ」と`ResultSetMetaData`
  （`getMetaData()`）から取れる「列メタデータ」を対で提供する構造をそのまま踏襲する。
  ```
  record ColumnMetadata(String columnName, String dataType, boolean nullable,
                         Integer primaryKeySequence, Permission effectivePermission)

  record RecordListResult(List<ColumnMetadata> columns, PageResult<List<Object>> records)

  MasterDataQueryService:
    RecordListResult listRecords(Long userId, Long connectionId, String schema, String table,
                                  FilterCriteria criteria, PageRequest page)
      // component-methods.md確定済みシグネチャ（戻り値 PageResult<RecordDto>）を
      // Functional Designで詳細化（U4 Q4のメソッド追加と同種の許容範囲）。
      // 独立した getColumnMetadata メソッドは導入しない。
  ```
  設計のポイント:
  - 実際に発行するSELECT文は`EffectivePermissionResolver.resolveEffectiveColumnPermissions`で
    NONE権限のカラムをSELECT句から**そもそも除外**する（UIで隠すだけでなく、権限のないデータを
    取得しない）。そのため`ResultSetMetaData`が返す列は常にR以上のカラムのみとなり、
    `columns`と`records`の列の食い違いが構造的に起こり得ない。
  - `columnName`/`dataType`/`nullable`は、その実行の`ResultSetMetaData`から導出する（U3
    インポート時点の`SchemaColumn`スナップショットに頼らず、対象RDBMS側の現在の構造を反映）。
  - `effectivePermission`（権限モデル由来）と`primaryKeySequence`（`ResultSetMetaData`では
    信頼できる形で取得できない）は、`EffectivePermissionResolver`/`SchemaColumn`から取得して
    付与する。
  - 各行は`List<Object>`（`columns`と同じ並び順の位置ベース）。列名を行ごとに繰り返し保持する
    マップ形式（当初案の`RecordDto`）は、`columns`側に列名が既にあるため冗長と判断し廃止する。
    値の型は`java.time.LocalDate`/`LocalDateTime`/`OffsetDateTime`（CLAUDE.md規約の`java.time`
    使用方針に準拠）、`BigDecimal`、`String`、`Boolean`等、`NamedParameterJdbcTemplate`の
    デフォルトの型マッピングをそのまま用いる。
  - **更新系（`MutationRequest`）は引き続きマップ形式を使う**（読み取り系との非対称設計）。
    `RecordCreate`/`RecordUpdate`は編集・入力された一部カラムのみを疎に表現する必要があり
    （全カラムを毎回位置指定で送るのは不自然）、`RecordUpdate`/`RecordDelete`の主キー特定も
    カラム名で明示する方が意図が明確なため。具体形はQ6で確定する。
  - U7（クエリ実行、未着手）の結果表示も同じ発想（`ResultSetMetaData`由来のカラム情報＋結果行を
    同梱する構造）になる想定で、U5とU7の設計思想を揃える申し送り事項とする。
...
- **A（推奨）**: `application.yml`に`mm.app.master-data.large-record-threshold`
  ...
  `listRecords`が返す`RecordListResult.records`（`PageResult<List<Object>>`、Q2確定）の実件数
  （1回のレスポンスに含まれる件数、ページサイズ）がこの閾値以上の場合に
  `AuditLogService.record(EventType.LARGE_RECORD_READ, ...)`を呼び出す（ページング自体の閾値＝
  1ページあたりの件数で判定し、テーブル全体の総件数では判定しない——「取得」という操作単位での
  大量データ取得を検知する趣旨のため）。
...
- [x] `aidlc-docs/construction/u5-master-data-maintenance/functional-design/domain-entities.md`
- [x] `aidlc-docs/construction/u5-master-data-maintenance/functional-design/business-rules.md`
- [x] `aidlc-docs/construction/u5-master-data-maintenance/functional-design/business-logic-model.md`
      （PBT-01: テスト可能な性質セクションを含む）
- [x] `aidlc-docs/construction/u5-master-data-maintenance/functional-design/frontend-components.md`