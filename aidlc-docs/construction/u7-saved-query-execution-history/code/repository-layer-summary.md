...
`QueryHistory`に`schema: String`列（`NOT NULL`、インデックスなし——`search`の絞り込み条件に
`schema`は含まれないため`nfr-design-patterns.md` 2の既存インデックス設計を変更する必要はない）を
追加した。`QueryHistoryRepository.search`のクエリ自体に変更はない。
