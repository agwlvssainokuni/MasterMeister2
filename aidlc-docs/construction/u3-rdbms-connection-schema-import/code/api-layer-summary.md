...
| パス | メソッド | 認可要件 | Controller |
|---|---|---|---|
| `/api/rdbms-connections/accessible` | GET | 認証必須のみ（`ROLE_ADMIN`不要、本ユニット唯一の例外） | `RdbmsConnectionController#listAccessibleConnections` |

`SecurityConfig`では`requestMatchers(HttpMethod.GET, "/api/rdbms-connections/accessible").authenticated()`を、既存の`requestMatchers("/api/rdbms-connections/**").hasRole("ADMIN")`より前に配置している（`requestMatchers`は最初にマッチしたルールが採用されるため順序が重要）。

`Authentication.getPrincipal()`を`userId`として`ConnectionAccessService.listAccessibleConnections(userId)`に渡す。成功時`200 OK`で`List<ConnectionSummary>`（`listConnections`と同じレスポンス形状）。

テストは`RdbmsConnectionControllerTest`に追加（一般ユーザーでの200確認、未認証401確認、計2件）。