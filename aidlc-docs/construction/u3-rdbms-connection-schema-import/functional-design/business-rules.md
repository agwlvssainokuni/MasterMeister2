...
### 1.7 アクセス可能な接続一覧の取得（2026-07-15変更要求、`ConnectionAccessService`）
`listAccessibleConnections(userId)`は、登録済み全接続のうち
`EffectivePermissionResolver.listAccessibleSchemas(userId, connectionId)`が1件以上の
スキーマを返す接続のみをフィルタして返す（`application-design/components.md`参照）。
- 対象RDBMSへのライブ接続は行わない。`listAccessibleSchemas`は内部DBに取り込み済みの
  スキーマメタデータ（`SchemaImportService`が取り込んだもの、2.1参照）を対象に権限判定する
  ため、接続不能な対象RDBMSがあっても本メソッドの呼び出しには影響しない。
- スキーマ未取り込みの接続（`SchemaImportService.importSchema`未実行）は、
  `listAccessibleSchemas`が常に空を返すため、本メソッドの結果からも常に除外される。
- 返す`ConnectionSummary`のフィールドは、既存の`listConnections`（管理者向け、1.x）と同一
  （`id`, `name`, `rdbmsType`, `host`, `databaseName`）。パスワード等の機密情報は含まない
  （既存の`listConnections`と同じ方針）。
- `masterdata`/`querybuilder`が個別に重複実装していた「アクセス可能な接続一覧」ロジックを
  本メソッドに一本化する（`stories.md` CHG-1、`component-dependency.md`注記1参照）。
...
**（2026-07-15変更要求）** 例外として、以下の新規エンドポイントのみ`hasRole("ADMIN")`を
要求しない。認証済み（`isAuthenticated()`）であれば一般ユーザー・管理者いずれも呼び出せる
（`stories.md` CHG-1のペルソナが「一般ユーザー、管理者」双方であるため）。

| パスパターン | 対象 | 認可 |
|---|---|---|
| `GET /api/rdbms-connections/accessible` | `listAccessibleConnections`（1.7） | `isAuthenticated()`（`hasRole("ADMIN")`は不要） |