...
`GET /connections`はStep 11（フロントエンド）着手時に判明した接続一覧API欠落
（U5「ブラウンフィールド発見事項」5と同種）への対応として、item 2-9/5-4/6-2/7-2で追加した。
**（2026-07-15変更要求）** `GET /connections`は削除した（詳細は下記）。
...
## `QueryBuilderController`（`/api/query-builder`）

**（2026-07-15変更要求）** `GET /connections`（選択可能接続一覧）は削除した。
`masterdata`と`querybuilder`が個別に重複実装していたロジック（`RdbmsConnectionRepository.
findAll()`を`EffectivePermissionResolver.listAccessibleSchemas`が非空の接続のみへ絞り込む）を
`rdbmsconnection.ConnectionAccessService.listAccessibleConnections`（U3所有、
`GET /api/rdbms-connections/accessible`）に一本化したため（`component-dependency.md`注記1、
Application Design Q3=B、U5と同様の対応）。`QueryBuilderMetadataService`から
`rdbmsConnectionRepository`フィールド自体（他メソッドで未使用だったため）も削除した。
フロントエンドの`QueryBuilderPage`はU1のグローバル接続コンテキスト（`useConnection()`）を
参照する形に変更した。

### `GET /{connectionId}/schemas`（選択可能スキーマ一覧）
...
### `GET /{connectionId}/schemas/{schema}/tables`（選択可能テーブル一覧）
...
### `GET /{connectionId}/schemas/{schema}/tables/{table}/columns`（選択可能カラム一覧、P1）
...
### `POST /{connectionId}/generate`（SQL生成、P2〜P6・P10）
...
### `POST /{connectionId}/parse`（SQL逆解析、P7〜P9）
...
| テストクラス | 検証内容 |
|---|---|
| `QueryBuilderControllerTest` | 5エンドポイント（`listSelectableSchemas`/`listSelectableTables`/`listSelectableColumns`/`generate`/`parse`）それぞれについて認証済みユーザ成功系・未認証401を検証（本ユニットは管理者ロール制約を持たないため403系テストは対象外、example-based、10件。2026-07-15変更要求で`listSelectableConnections`関連2件を削除） |
