...
|---|---|---|---|
| `/api/query-builder/connections` | GET | 認証必須（ロール制約なし） | `QueryBuilderController#listSelectableConnections` |
...
### `GET /connections`（選択可能接続一覧）

ボディなし。`QueryBuilderMetadataService.listSelectableConnections(userId)`
（item 2-9で新規追加——U5「ブラウンフィールド発見事項」5と同種の問題、`frontend-components.md`の
`QueryBuilderPage`は接続選択を内包する設計だが、Step 5-1〜5-3確定時点の`querybuilder`
パッケージには接続一覧を列挙する手段がなかったため、Step 11着手時にユーザ指示によりU5と同一
パターンで解決した。`RdbmsConnectionRepository.findAll()`の全接続を
`effectivePermissionResolver.listAccessibleSchemas(userId, connectionId)`が非空のもの
...
| テストクラス | 検証内容 |
|---|---|
| `QueryBuilderControllerTest` | 6エンドポイント（`listSelectableConnections`/`listSelectableSchemas`/`listSelectableTables`/`listSelectableColumns`/`generate`/`parse`）それぞれについて認証済みユーザ成功系・未認証401を検証（本ユニットは管理者ロール制約を持たないため403系テストは対象外、example-based、12件） |
