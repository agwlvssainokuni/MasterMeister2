...
| ファイル | 内容 |
|---|---|
| `types.ts` | `TableRef`/`ColumnRef`/`FromItem`/`JoinType`/`AggregateFunction`/`Operator`/`SortDirection`/`Condition`/`JoinItem`/`SelectItem`/`OrderByItem`/`QueryBuilderModel`/`GeneratedSql`/`ParseResult`。`SortDirection`は`masterData`/`schema`と同様、本feature内にローカル再定義（他feature非依存の方針）。**（2026-07-15変更要求）** `ConnectionSummary`/`RdbmsType`（Step 11-3で追加していたもの）は削除した |
| `api.ts` | `listSelectableSchemas(connectionId)` → `GET /api/query-builder/{connectionId}/schemas`、`listSelectableTables(connectionId, schema)` → `GET .../schemas/{schema}/tables`、`listSelectableColumns(connectionId, schema, table)` → `GET .../tables/{table}/columns`、`generateSql(connectionId, model)` → `POST .../generate`、`parseSql(connectionId, rawSql)` → `POST .../parse`。U1の`apiClient`（`apiFetch`）を再利用。**（2026-07-15変更要求）** `listSelectableConnections()`（`GET /api/query-builder/connections`、Step 11-3実装時に追加していたもの）は削除した（`rdbmsConnection/api.ts`の`listAccessibleConnections()`＝`GET /api/rdbms-connections/accessible`へ一本化） |
| `QueryBuilderPage.tsx` | **（2026-07-15変更要求により改訂）** 接続はU1の`useConnection()`（グローバル接続コンテキスト）から取得し、ページ内接続選択UIは持たない。`connectionId`が`null`の場合は「接続が指定されていません。」を表示する。スキーマ選択、7タブ（FROM/JOIN・SELECT・WHERE・GROUP BY・HAVING・ORDER BY・LIMIT/OFFSET）切り替えコンテナ。`fromItem`/`joinItems`/`selectItems`/`whereConditions`/`groupByColumns`/`havingConditions`/`orderByItems`/`limit`/`offset`の各stateを保持し子タブへ配布する。`fromItem`確定後のみ`GeneratedSqlPanel`を常時表示（タブ切替に関わらず）し、`SqlReverseParsePanel`は接続コンテキストが確立していれば（スキーマ選択前でも）常時表示する。URLクエリパラメータ`rawSql`（U7からの遷移、GEN-9）をマウント時に読み取り自動でSQL解析を行う。URLクエリパラメータ`schema`（新規）を初回マウント時のみ読み取り、スキーマの初期値としてプリフィルする（画面内で上書き可能）。`connectionId`が初回マウント後に変化した場合、`schema`・組み立て中の`model`・`generatedSql`をリセットする（`useRef`で初回マウント判定、`stories.md` CHG-2） |
...
## 実装時に追加したAPI（Step 11-3着手時の発見・対応、2026-07-15変更要求で削除済み）

`frontend-components.md`のQueryBuilderPageは接続・スキーマ選択を内包する設計だったため、
Step 5-1〜5-3確定時点の`querybuilder`パッケージには接続一覧を列挙するAPIが存在しなかった
（U5「ブラウンフィールド発見事項」5と同種の問題）。当時はユーザ指示によりU5と同一パターンで
解決し、バックエンド側にitem 2-9（`QueryBuilderMetadataService.listSelectableConnections`）・
5-4（`GET /api/query-builder/connections`）・6-2・7-2を追加していた。**2026-07-15変更要求により
接続選択がグローバルコンテキスト化されたため、これらはすべて削除した**（詳細は
...
`query-builder-page`, `query-builder-page-schema-select`,
...
- **接続一覧APIの欠落**: 上記のとおり。U5と同一パターンで解決（既存の承認済みStep 5〜7を
  差し戻すのではなく、item追加として対応）。**2026-07-15変更要求により、この対応自体を撤回し
  グローバル接続コンテキストへ一本化した**（上記「実装時に追加したAPI」節参照）。
...
- **`SqlReverseParsePanel`の表示条件**: `parse` APIは`connectionId`のみを要求し`schema`を
  ...
  スキーマ選択欄の直後に配置した。
- **URLクエリパラメータ連携**: `rawSql`（`frontend-components.md`で言及済み）に加え、当初は
  `connectionId`もURLクエリパラメータとして読み取り、マウント時の自動接続選択に用いる設計を
  実装時に追加していた（U7からの遷移時、対象接続が既知であることが自然なため）。
  **2026-07-15変更要求により、接続はグローバル接続コンテキスト（U1）に一本化されたため
  URL経由の`connectionId`受け取りは廃止した**。代わりに`schema`をURLクエリパラメータとして
  受け取り、初回マウント時のみスキーマの初期値としてプリフィルする設計に変更した。

## テストカバレッジ（Step 12）

| テストファイル | 件数 | 検証内容 |
|---|---|---|
| `QueryBuilderPage.test.tsx` | 11 | **（2026-07-15変更要求により改訂）** `connectionId`未設定時のメッセージ表示、グローバル接続コンテキストの`connectionId`に基づくスキーマロードとタブ非表示、`schema`URLパラメータからのプリフィル、接続切替時のスキーマ・モデルリセット、スキーマ選択後のタブ表示（FromJoinTabが既定）、タブ切替、`fromItem`未確定時の`GeneratedSqlPanel`非表示、ベーステーブル選択後の表示、SQL生成ボタン押下→表示、保存/実行遷移時の`connectionId`/`schema`/`rawSql`引き継ぎ（`MemoryRouter`でラップ、`useConnectionStore.setState`で`connectionId`を直接設定） |
...
- `QueryBuilderPage.tsx`をグローバル接続コンテキスト参照に改修し、接続選択UIとページ内
  `connectionId`/`connections` stateを削除。`connectionId===null`時は「接続が指定されて
  いません。」を表示。`connectionId`変化時（初回マウント後のみ）に`schema`/`model`/
  `generatedSql`をリセットする`useEffect`を追加（`useRef`で初回マウント判定）。
- `schema`のURLパラメータプリフィル（受信、初回マウント時のみ）、`handleNavigateToSave`/
  `handleNavigateToExecute`への`schema`パラメータ追加（送信）を実装。
- `queryBuilder/api.ts`から`listSelectableConnections`、`queryBuilder/types.ts`から
  `ConnectionSummary`/`RdbmsType`を削除。
- バックエンド: `QueryBuilderController`/`QueryBuilderMetadataService`から
  `listSelectableConnections`/`GET /connections`を削除（`rdbmsConnectionRepository`
  フィールド自体も他メソッド未使用だったため削除）。
- `QueryBuilderPage.test.tsx`を改訂（6件→11件）。`npx vitest run`（フロントエンド全体）
  274/274件成功、`./gradlew test --tests "cherry.mastermeister.querybuilder.*"`成功、
  `./gradlew build -x test`・`npx tsc -b`・`npx oxlint`いずれもエラーなしを確認済み。