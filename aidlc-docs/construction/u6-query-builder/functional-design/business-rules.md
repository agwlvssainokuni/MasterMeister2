...
- JOIN句・WHERE句・GROUP BY/HAVING句・ORDER BY句の組み立てロジックは`querybuilder`側に
  新規実装する
- テーブル参照（FROM/JOIN）にスキーマ修飾は付与しない（方言によらず、常に
  `dialect.quoteIdentifier(table)`のみ）。生成SQLを別環境（dev/stg/prod等、同一スキーマ構成の
  別データベース）でもそのまま再利用できるようにするための設計判断（U7 Functional Design着手時に
  訂正、それ以前はPostgreSQL/H2でスキーマ修飾を付与していた）。対象スキーマが実行時の接続の
  既定スキーマ（`search_path`等）と異なると解決に失敗し得るが、このリスクは許容する。
