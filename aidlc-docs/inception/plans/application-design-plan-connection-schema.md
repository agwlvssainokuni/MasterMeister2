...
- [x] Step A: `rdbmsconnection`パッケージに新規`ConnectionAccessService`
      （`listAccessibleConnections(userId)`）を追加する設計を確定した
- [x] Step B: `queryexecution`パッケージのスキーマ許可リスト検証の依存先を
      `permission.EffectivePermissionResolver.listAccessibleSchemas`に確定した
- [x] Step C: 新規エンドポイント`GET /api/rdbms-connections/accessible`を確定し、
      既存の`/api/master-data/connections`・`/api/query-builder/connections`は廃止することとした
- [x] Step D: `component-dependency.md`のマトリクスを更新した（`rdbmsconnection → permission`、
      `queryexecution → permission`の新規依存エッジ追加、注記1・注記2を追加、通信パターンに
      新規エンドポイントの説明を追加、データフロー図1にノード・エッジを追加）
- [x] Step E: `components.md`/`component-methods.md`/`services.md`/`application-design.md`を
      更新した（新規コンポーネント`ConnectionAccessService`、`QueryExecutionService`のスキーマ
      検証・`SET search_path`発行、`SavedQueryService`のスキーマ非保存、
      `QueryHistoryService`のスキーマ記録、フロー8「グローバル接続コンテキストの解決」追加）
