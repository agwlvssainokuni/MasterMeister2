...
- アクセス権限（テーブルレベル Allow/Deny、カラムレベル なし/R/RU/CRUD）は
  「接続 ×（ユーザ or ユーザグループ）× テーブル/カラム」単位で設定する（3.2, 3.3参照）。
...
## 8. 未解決・後続フェーズで検討する事項
- グループ権限とユーザ個別権限の優先順位・合成ルールの詳細（3.2） → Application Design
- 認証トークンの設定キー名・デフォルト有効期限（3.5） → Application Design / NFR Design
- i18nライブラリの技術選定（3.6） → NFR Design
- 複数RDBMS接続を跨いだ管理UI/UXの詳細（3.3） → User Stories / Application Design