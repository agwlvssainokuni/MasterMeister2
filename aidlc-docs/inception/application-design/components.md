...
| コンポーネント | 種別 | 責務 |
|---|---|---|
| `RdbmsConnectionService` | サービス | 対象RDBMS接続情報（種別/ホスト/ポート/DB名/認証情報等）のCRUD、接続テストを担う（管理者向け）。 |
...
| `ConnectionAccessService` | サービス（Facade、2026-07-15変更要求で追加） | ログインユーザー（一般ユーザー含む）がアクセス可能な対象RDBMS接続の一覧を提供する。`EffectivePermissionResolver.listAccessibleSchemas` の結果が1件以上ある接続のみを返す（`masterdata`/`querybuilder`がそれぞれ個別に重複実装していたロジックを一本化）。フロントエンドのグローバル接続セレクタ（`AppLayout`常設）が唯一の呼び出し元となる想定。 |
...
| `SchemaQueryService` | サービス | 取り込み済みスキーマ情報の参照（スキーマ一覧、テーブル一覧、テーブル詳細=カラム一覧・主キー構成等）を提供する。権限フィルタは持たない生の一覧であり、`permission`・`masterdata`・`querybuilder` がそれぞれ `EffectivePermissionResolver` の判定結果でフィルタして利用する。 |
...
| `EffectivePermissionResolver` | サービス（Facade） | ユーザ・接続・スキーマ/テーブル/カラムを与えて実効権限を解決する中心コンポーネント（Question 3 = A）。グループ合成（最も緩い権限を採用）・個別上書き（同じ階層上書き機構）・主権限の階層継承・補助権限と主キー権限の組合せ判定（作成/削除可否、複合主キーAND条件、主キー無しテーブルの作成のみ例外）に加え、アクセス可能なスキーマ一覧・テーブル一覧のフィルタ（`listAccessibleSchemas` / `listAccessibleTables`）を担う。`masterdata` / `querybuilder` / `queryexecution` は本サービスのAPIのみを呼び出し、`permission` の内部エンティティには直接アクセスしない。 |
...
| コンポーネント | 種別 | 責務 |
|---|---|---|
| `MasterDataQueryService` | サービス | アクセス可能スキーマ一覧・テーブル一覧、レコード一覧（絞り込み・ソート・ページング）を提供する。`EffectivePermissionResolver` によるスキーマ/テーブル/カラム読み取り権限フィルタを適用する。手入力WHERE/ORDER BYはカラム権限フィルタの対象外（GEN-2、設計上の意図的例外）。 |
...
| コンポーネント | 種別 | 責務 |
|---|---|---|
| `QueryBuilderMetadataService` | サービス | スキーマ選択、FROM/JOINタブでのテーブル選択、他タブでのアクセス可能カラム一覧提供（`EffectivePermissionResolver` 経由の読み取り権限フィルタ適用）。 |
...
| コンポーネント | 種別 | 責務 |
|---|---|---|
| `SavedQueryService` | サービス | SQL（手入力またはクエリビルダー生成）への名前付け保存、公開/非公開設定、一覧・詳細取得（可視性チェック）、作成者のみに限定した編集を担う（GEN-10, GEN-11, GEN-12）。**（2026-07-15変更要求）** スキーマは保存しない（SQLがスキーマ非依存で再利用可能というクエリビルダーの設計意図を維持するため。実行のたびに`queryexecution`側でスキーマを指定する）。 |
...
| コンポーネント | 種別 | 責務 |
|---|---|---|
| `QueryExecutionService` | サービス | 手入力SQL・保存クエリの実行を担う。読み取り専用SQLのみ許可する簡易検証（DML/DDLキーワード検知等）、`:param` 形式パラメータの自動検出と `NamedParameterJdbcTemplate` へのバインド、ページング制御を行う。実行のたびに `QueryHistoryService.recordExecution` と `AuditLogService.record` の両方を明示的に呼び出す。**（2026-07-15変更要求）** 実行対象のスキーマを必須パラメータとして受け取り、`EffectivePermissionResolver.listAccessibleSchemas` による許可リスト検証を経てから、`SCHEMA_BASED`方言（PostgreSQL/H2）の場合のみ実行直前に検証済みスキーマ名で`SET search_path`を発行する（`CATALOG_BASED`方言のMySQL/MariaDBは元々1スキーマ固定のため対応不要）。テーブル/カラム単位の読み取り権限フィルタは引き続き適用しない（既存の設計上の受容リスク、`component-dependency.md`参照）。 |
...
| コンポーネント | 種別 | 責務 |
|---|---|---|
| `QueryHistoryService` | サービス | クエリ実行履歴の記録（SQL・パラメータ・結果件数・実行時間・実行回数・実行日時・実行者、**（2026-07-15変更要求）実行対象スキーマ**）、一覧・絞り込み（実行日時範囲・実行者・SQLテキスト検索）を担う。`queryexecution` から呼び出される。 |
...
- **アクセス可能な接続一覧の一本化（2026-07-15変更要求）**: `masterdata`/`querybuilder`が個別に
  重複実装していた「アクセス可能な接続一覧」取得ロジックを `rdbmsconnection.ConnectionAccessService`
  に一本化する。フロントエンドは`AppLayout`常設のグローバル接続セレクタ1箇所からのみこれを呼び出す
  想定で、`masterdata`/`querybuilder`のページ内接続セレクタは廃止する（`stories.md` CHG-1参照）。
- **クエリ実行時のスキーマ指定（2026-07-15変更要求）**: `queryexecution`は実行対象のスキーマを
  必須パラメータとして受け取り、`permission.EffectivePermissionResolver.listAccessibleSchemas`で
  許可リスト検証してから実行する。これは`queryexecution`にとって新規の`permission`への依存であり、
  既存の「テーブル/カラム単位の読み取り権限フィルタは適用しない」という設計決定（下記
  `component-dependency.md`の注記）とは独立した、スキーマ選択肢の妥当性検証のみを目的とする
  限定的な依存である。
