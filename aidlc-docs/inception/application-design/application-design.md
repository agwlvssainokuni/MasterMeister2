...
- `queryexecution`（手入力SQL・保存クエリ実行）が、テーブル/カラム単位の読み取り権限フィルタ
  目的では引き続き `permission` パッケージに依存しない設計となっている点（`component-dependency.md`
  注記参照。2026-07-15変更要求でスキーマ検証目的の限定的な`permission`依存は追加されたが、
  この論点自体は解消されていない）。`stories.md` GEN-13 の受け入れ基準に忠実な設計だが、
  対象RDBMSの任意テーブル/カラムを権限に関わらず読み取れることを意味するため、
  `security-baseline` 拡張のオプトイン時、または後続のNFR Requirements段階で改めて要否を
  確認することを推奨する。
- 認証トークン設定キー: `mm.app.auth.token-expiry-hours`（デフォルト値は8時間を提案、
  最終値はNFR Designで確定）。

## 変更要求: 接続コンテキストのグローバル化 + クエリ実行時スキーマ指定（2026-07-15）

`requirements.md` §9 で確定した変更要求に対応する、Application Designレベルの決定（詳細は
`components.md`・`component-methods.md`・`services.md`・`component-dependency.md`の各
「2026-07-15変更要求」注記を参照）:

| # | 論点 | 決定 |
|---|---|---|
| CHG-Q1 | アクセス可能な接続一覧ロジックの配置 | `rdbmsconnection`パッケージに新規`ConnectionAccessService`を切り出す（`RdbmsConnectionService`は管理者向けCRUD専任のまま）。`masterdata`/`querybuilder`の重複実装を解消。 |
| CHG-Q2 | `queryexecution`のスキーマ許可リスト検証の依存先 | `permission.EffectivePermissionResolver.listAccessibleSchemas`を呼び出す新規依存（`queryexecution → permission`）。テーブル/カラム単位の読み取り権限フィルタとは別軸の、スキーマ選択肢の妥当性検証のみを目的とする限定的な依存。 |
| CHG-Q3 | 新規エンドポイントのパス | `GET /api/rdbms-connections/accessible`を新設。既存の`GET /api/master-data/connections`・`GET /api/query-builder/connections`は廃止。 |

**コンポーネント構成への影響**: 新規コンポーネント`rdbmsconnection.ConnectionAccessService`
（1件）を追加。既存コンポーネントの責務変更（`QueryExecutionService`にスキーマ検証・
`SET search_path`発行、`SavedQueryService`はスキーマ非保存、`QueryHistoryService`は
スキーマ記録）。新規コンポーネントの追加なので、フロントエンドには対応する「グローバル接続
セレクタ」という新しいUI構造要素が生まれる（`aidlc-docs/inception/user-stories/stories.md`
CHG-1参照、詳細はFunctional Designで確定）。
